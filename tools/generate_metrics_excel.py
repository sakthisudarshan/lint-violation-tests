...
PYLINT_JSON = REPORTS_DIR / "pylint_output_raw.json"
PYLINT_JSON_MAIN = REPORTS_DIR / "pylint_output.json"
PYLINT_JSON_PLUGIN = REPORTS_DIR / "pylint_with_plugin.json"
...
def _run_pylint_on(target: Path, rcfile: Path, extra_args: list | None = None) -> list[dict]:
    ...
    env = os.environ.copy()
    pypath_parts = [str(PLUGINS_DIR), str(ROOT_DIR), str(SRC_DIR)]
    existing_pypath = env.get("PYTHONPATH", "")
    if existing_pypath:
        pypath_parts.append(existing_pypath)
    env["PYTHONPATH"] = os.pathsep.join(pypath_parts).strip(os.pathsep)
...
def run_pylint() -> list[dict]:
    ...
    1. Main codebase scan on src/ using the project .pylintrc. This clean scan is
       stored in pylint_output_raw.json so the taxonomy gate sees main-codebase metrics.
    ...
    """
    main_rcfile = ROOT_DIR / ".pylintrc"
    sample_rcfile = ROOT_DIR / ".pylintrc-sample"

    # --- Scan 1: main codebase quality scan (src/ only) ---
    scan_target = SRC_DIR if SRC_DIR.exists() else ROOT_DIR
    main_violations = _run_pylint_on(scan_target, main_rcfile)
...
def compute_metrics(violations: list[dict]) -> dict:
    ...
    config_present   = 1 if (ROOT_DIR / ".pylintrc").exists() else 0
...
if __name__ == "__main__":
    ...
    out_file = REPORTS_DIR / "Pylint_Lint_Violation_Metrics_R27_R38.xlsx"
